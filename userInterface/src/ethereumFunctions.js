...
export async function getBalanceAndSymbol(
  ...
  try {
    if (address === COINS.WETH.address) {
      const balanceRaw = await provider.getBalance(accountAddress);

      return {
        balance: ethers.utils.formatEther(balanceRaw),
        symbol: COINS.WETH.abbr,
      };
    } else {
...
// This function swaps two particular tokens / WETH, it can handle switching from WETH to ERC20 token, ERC20 token to WETH, and ERC20 token to ERC20 token.
...
export async function swapTokens(
  ...
  if (address1 === COINS.WETH.address) {
    // Eth -> Token
  ...
  } else if (address2 === COINS.WETH.address) {
    // Token -> Eth
...
//    `address1` - An Ethereum address of the token to swaped from (either a token or WETH)
//    `address2` - An Ethereum address of the token to swaped to (either a token or WETH)
...
//    `address1` - An Ethereum address of the token to trade from (either a ERC20 token or WETH)
//    `address2` - An Ethereum address of the token to trade to (either a ERC20 token or WETH)
...
//    `address1` - An Ethereum address of the token to trade from (either a token or WETH)
//    `address2` - An Ethereum address of the token to trade to (either a token or WETH)
